...
export default class CrowdSaleContract extends BaseInterface {
    ...
    async buyTokenByBNB(amount: number){
        ...
        const tx: TransactionResponse = await this._contract.buyTokenByBNB({...this._option, value:this._numberToEth(amount/rate)})
        return this._handleTransactionResponse(tx);
    }

    async buyTokenByUSDT(amount: number){
        const rate = await this.getUsdtRate();
        const tx: TransactionResponse = await this._contract.buyTokenByUSDT(this._numberToEth(amount/rate), this._option)
        return this._handleTransactionResponse(tx);
    }
}